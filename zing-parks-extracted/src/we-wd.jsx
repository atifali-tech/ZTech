// Weekend vs Weekday — clustered bars. One section with two charts.
const ClusteredBars = ({ data, valueKey1, valueKey2, label1, label2, color1, color2, formatter, height = 240 }) => {
  ...
  return (
    <div ref={ref} className="chart-area">
      <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
        ...
        {[0, 0.5, 1].map((t,i) => (
          <text key={i} x={pad.l-8} y={pad.t+innerH*(1-t)+3} fontSize="10" fill="var(--ink-4)" textAnchor="end" fontFamily="'JetBrains Mono', monospace">{formatter(max*t)}</text>
        ))}
        {data.map((d, i) => {
          ...
          return (
            <g key={i} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)}>
              ...
              {isHover && (
                <>
                  <text x={x1+barW/2} y={y1-4} fontSize="9.5" fill="var(--ink)" textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight="600">{formatter(d[valueKey1])}</text>
                  <text x={x2+barW/2} y={y2-4} fontSize="9.5" fill="var(--ink)" textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight="600">{formatter(d[valueKey2])}</text>
                </>
              )}
...
const WeWdSection = () => (
  <div className="grid-12">
    <div className="col-6">
      ...
      >
        <ClusteredBars data={WE_WD_REV} valueKey1="weekend" valueKey2="weekday" label1="Weekend" label2="Weekday" color1="#0E7C66" color2="#A0CFC2" formatter={inr}/>
      </Section>
    </div>
    <div className="col-6">
      <Section
      ...
      >
        <ClusteredBars data={WE_WD_FF} valueKey1="weekend" valueKey2="weekday" label1="Weekend" label2="Weekday" color1="#5A6BCF" color2="#BAC2EC" formatter={num}/>
      </Section>
    </div>
  </div>
);

Object.assign(window, { WeWdSection });
