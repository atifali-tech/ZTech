// Comparative: QvQ (4 quarters) and YvY (12 months)
const QvQChart = () => {
        ...
        {QvQ.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={cx} y={yAt(d.curr)-4} fontSize="9.5" fill={change>=0?"var(--good)":"var(--bad)"} textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight="600">{change>=0?"+":""}{change.toFixed(1)}%</text>
        ...
        {YvY.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              {i%2===0 && <text x={cx} y={yAt(d.curr)-4} fontSize="8.5" fill={change>=0?"var(--good)":"var(--bad)"} textAnchor="middle" fontFamily="'JetBrains Mono', monospace" fontWeight="600">+{change.toFixed(0)}%</text>}
...
const ComparativeSection = ({ visible }) => {
  if (!visible) {
    return (
      <Section
    ...
    <div className="grid-12">
      <div className="col-6">
        <Section
          ...
          sub="FY26 vs FY25"
          actions={<span style={{display:"inline-flex",gap:10,alignItems:"center",fontSize:11,color:"var(--ink-3)"}}>
...
Object.assign(window, { ComparativeSection });
